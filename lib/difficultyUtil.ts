...
export function calculateDifficulty(promptText: string, category: string): {
  ...
  const longestWord = promptText.split(' ').reduce((a, b) => (b.length > a.length ? b : a), '');
  // ≤4 letters: 1, 5-6:2, 7-8:3, 9-10:4, >10:5
  const freqNorm = Math.max(1, Math.min(5,
    longestWord.length <= 4 ? 1 : longestWord.length <= 6 ? 2 : longestWord.length <= 8 ? 3 : longestWord.length <= 10 ? 4 : 5
  ));
  ...
  const lengthScale = length <= 3 ? 1 : length <= 6 ? 2 : length <= 9 ? 3 : length <= 12 ? 4 : 5;

  // Response type: open=3, personal=2, else=1
  const responseTypeScale = category === 'open' ? 3 : category === 'personalVocab' ? 2 : 1;

  // Semantic distance: open|personal=1, generic=2, challenge=3
  const semanticDistanceScale = category === 'personalVocab' || category === 'open' ? 1 : category === 'genericVocab' ? 2 : 3;
