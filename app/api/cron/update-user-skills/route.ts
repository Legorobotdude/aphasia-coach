...
export async function GET() {
  try {
    ...
    for (const user of users) {
      const uid = user.id;
      // Use type assertion to handle potential missing skillScores
      ...
      const categories = ['genericVocab', 'personalVocab', 'challenge'] as const;
    ...
    return NextResponse.json({ message: 'Nightly update completed successfully' }, { status: 200 });
  } catch (error) {
    console.error('Error in nightly update:', error);
    return NextResponse.json({ error: 'Failed to complete nightly update' }, { status: 500 });
  }
} 