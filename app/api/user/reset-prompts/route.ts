...
export async function POST(req: NextRequest) {
  try {
    ...
    const q = promptPoolRef.orderBy('__name__');  // Use a basic query if needed, or just get all docs
    ...
    return NextResponse.json({ message: 'Prompts reset successfully.' }, { status: 200 });
  } catch (error) {
