...
export async function POST(req: NextRequest) {
  try {
    ...
    if (text.length > 4096) { // OpenAI's TTS has a limit of 4096 characters
        console.log('[API /tts] Bad request: Text input exceeds maximum length');
        return NextResponse.json(
            { error: 'Bad request: Text input exceeds maximum length of 4096 characters.' },
            { status: 400 }
        );
    }

    console.log(`[API /tts] Received text for synthesis. Length: ${text.length}, Voice: ${voice || 'default'}`);
    ...
    if (!audioArrayBuffer || audioArrayBuffer.byteLength === 0) {
        console.error('[API /tts] Speech synthesis failed or returned empty audio.');
        return NextResponse.json(
    ...
    console.log(`[API /tts] Speech synthesis successful. Audio size: ${audioArrayBuffer.byteLength} bytes.`);
    ...
    if (error.message && (error.message.includes('OpenAI API Error') || error.status)) {
        return NextResponse.json(
            { error: `Speech synthesis service error: ${error.message}` },
