...
function sessionReducer(state: State, action: Action): State {
  switch (action.type) {
    case 'FETCH_PROMPTS_START':
      ...
      return { ...state, status: 'FETCHING_PROMPTS', error: null, completedUtterances: [], sessionStartedAt: null };
    ...
    case 'START_PROMPT':
       if (state.status === 'SESSION_READY') { // Check if session is ready
...
export default function VoiceSession() {
  ...
  const latencyTimerRef = useRef<NodeJS.Timeout | null>(null); // Ref for latency timer
  ...
  const {
    ...
    onDataAvailable: (blob) => {
      if (state.status === 'RECORDING' && user && state.sessionId && currentPrompt) {
         // Stop latency timer *before* processing
         const endTime = Date.now();
         const latencyMs = state.recordingStartTime ? endTime - state.recordingStartTime : null;
  ...
  const { data: promptsData, error: promptsError, isLoading: promptsLoading } = useSWR(
  ...
  useEffect(() => {
    ...
    if (promptsData && state.status === 'FETCHING_PROMPTS') {
        ...
        if (promptsData && Array.isArray(promptsData.prompts)) {
          dispatch({ type: 'FETCH_PROMPTS_SUCCESS', payload: promptsData.prompts });
        } else {
           console.error('Invalid prompts data structure received:', promptsData);
           dispatch({ type: 'FETCH_PROMPTS_ERROR', payload: 'Invalid data received from prompts API' });
        }
    }
  ...
  useEffect(() => {
      ...
      if (state.status === 'INITIALIZING_SESSION' && state.sessionId && user && !state.sessionDocCreated) {
          ...
          console.log('Attempting to create initial session document...', state.sessionId);
          setDoc(doc(db, 'sessions', state.sessionId), initialSessionData)
              ...
              .catch((error) => {
                  ...
                  dispatch({ type: 'FETCH_PROMPTS_ERROR', payload: 'Failed to initialize session.' });
              });
      }
  }, [state.status, state.sessionId, user, state.sessionDocCreated, state.prompts.length, dispatch]); // Added dispatch dependency
  ...
  useEffect(() => {
    if (recorderError) {
      ...
      dispatch({ type: 'PROCESSING_ERROR', payload: `Recording failed: ${recorderError.message}` });
    }
  }, [recorderError]);
    ...
    if (state.status === 'SESSION_READY' && currentPrompt?.text) {
      ...
      try {
        ...
        utterance.onerror = (event) => {
            ...
            dispatch({ type: 'FETCH_PROMPTS_ERROR', payload: `Speech synthesis error: ${event.error}` }); 
        };
        ...
        window.speechSynthesis.speak(utterance);
         ...
         dispatch({ type: 'FETCH_PROMPTS_ERROR', payload: 'Failed to start speech synthesis' });
      }
      // --- End TTS Integration --- //
  ...
  const handleStartRecording = useCallback(() => {
    if (state.status === 'RECORDING') { // Or READY_TO_RECORD
      console.log('Dispatching START_RECORDING');
      dispatch({ type: 'START_RECORDING' });
      // TODO: Integrate useRecorder hook
    }
  }, [state.status]);
  ...
  const handleProcessRecording = useCallback(async (blob: Blob | null, latencyMs: number | null) => {
    if (!blob) {
        console.error('Processing stopped: No blob received from recorder.');
        dispatch({ type: 'PROCESSING_ERROR', payload: 'No audio data received.' });
        return;
    }
    if (state.status !== 'RECORDING' || !user || !state.sessionId || !currentPrompt) {
        console.warn('Processing stopped: Invalid state or missing data.', { status: state.status, user: !!user, sessionId: state.sessionId, currentPrompt: !!currentPrompt });
    ...
    try {
      // 1. Transcribe
      console.log(`Transcribing blob of size ${blob.size}, type ${blob.type}`);
      ...
      if (!transcribeRes.ok) {
        ...
        throw new Error(`Transcription failed: ${transcribeRes.status} ${transcribeRes.statusText} - ${errorBody.error || 'Unknown error'}`);
      }
      ...
      const scoreRes = await fetch('/api/openai/score', {
        ...
        body: JSON.stringify({ prompt: currentPrompt.text, response: transcript }),
        // Auth handled by session cookie
      });
      if (!scoreRes.ok) {
         const errorBody = await scoreRes.json().catch(() => ({}));
        throw new Error(`Scoring failed: ${scoreRes.status} ${scoreRes.statusText} - ${errorBody.error || 'Unknown error'}`);
      }
      ...
      const utteranceData: Omit<Utterance, 'id' | 'createdAt'> & { createdAt: any } = {
      ...
      const newUtteranceDocRef = await addDoc(utterancesCollectionRef, utteranceData);
      ...
      dispatch({ type: 'PROCESSING_SUCCESS', payload: { ...utteranceData, id: newUtteranceDocRef.id } });

    } catch (err: any) {
      console.error('Processing error:', err);
      dispatch({ type: 'PROCESSING_ERROR', payload: err.message || 'Failed to process recording' });
    }
  }, [state.status, user, currentPrompt, state.sessionId, dispatch]); // Added dispatch to dependencies
  ...
  const handleNext = useCallback(async () => {
    ...
    if (state.status === 'COMPLETE' && user && state.sessionId && state.sessionStartedAt) {
        ...
        if (completed.length > 0) {
            ...
            meanLatencyMs = validLatencyCount > 0 ? totalLatency / validLatencyCount : 0;
        }

        const sessionEndTime = Date.now();
        const durationSec = Math.round((sessionEndTime - state.sessionStartedAt) / 1000);
        ...
        try {
            ...
            await setDoc(doc(db, 'sessions', state.sessionId), sessionSummary, { merge: true });
  ...
  }, [state.status, state.sessionId, user, state.prompts.length, state.sessionStartedAt]);

  // --- Render Logic --- //

  if (state.status === 'FETCHING_PROMPTS' || state.status === 'INITIALIZING_SESSION' || promptsLoading) {
    return <div>Loading session...</div>; // Updated loading message
  }

  if (state.status === 'ERROR') {
    return <div>Error: {state.error} <button onClick={() => dispatch({type: 'RESET'})}>Retry</button></div>;
  }

  if (!currentPrompt && (state.status !== 'IDLE' && state.status !== 'COMPLETE')) {
      // This might happen if promptsData is empty or structure is wrong
      if (!promptsLoading && !promptsError) {
          return <div>No prompts available or failed to load. <button onClick={() => dispatch({type: 'RESET'})}>Try Again</button></div>;
      }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4">
      ...
      {(state.status === 'SESSION_READY' || state.status === 'PLAYING_PROMPT' || state.status === 'RECORDING' || state.status === 'PROCESSING' || state.status === 'FEEDBACK') && (
         <PromptCard 
            promptText={currentPrompt?.text || (state.status === 'SESSION_READY' ? 'Ready...' : 'Loading...')}
      ...
      {state.status === 'COMPLETE' && (
          <div className="my-4 p-4 border rounded shadow bg-yellow-100">
              <p>Session Complete!</p>
               <button onClick={handleNext} className="mt-2 px-4 py-2 bg-yellow-500 text-black rounded">
                  Go to Dashboard
               </button>
